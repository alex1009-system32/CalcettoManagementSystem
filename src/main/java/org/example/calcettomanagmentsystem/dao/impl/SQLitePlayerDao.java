...
public class SQLitePlayerDao implements PlayerDao {
    ...
    public List<Player> findAll() {
        String sql =
                "SELECT p.pid, p.pname, p.pemail, t.tid, t.tournament_name, t.start_date, t.duration, t.pre_round, t.current_round, t.max_team_size FROM player p JOIN tournament t ON t.tid = p.trid";
    ...
    public List<Player> getAllPlayersFromTournament(@NotNull Tournament tournament) {
        String sql =
                "SELECT p.pid, p.pname, p.pemail, t.tid, t.tournament_name, t.start_date, t.duration, t.pre_round, t.current_round, t.max_team_size FROM player p JOIN tournament t ON t.tid = p.trid WHERE t.tid = ?";
        List<Player> players = new ArrayList<>();
    ...
    public Optional<Player> findById(int id) {
        String sql =
                "SELECT p.pid, p.pname, p.pemail, t.tid, t.tournament_name, t.start_date, t.duration, t.pre_round, t.current_round, t.max_team_size FROM player p JOIN tournament t ON t.tid = p.trid WHERE p.pid = ?";
        Player player = null;
